              ...
              <div class="custom-select-wrapper">
                  <div class="custom-select">
                      ...
                      <div class="custom-options">
                          <?php
                            $platforms_query = "SELECT p.name, p.logo FROM platforms p ORDER BY p.sort_order ASC";
                            ...
                            if ($platforms_result && $platforms_result->num_rows > 0) {
                                while ($platform_row = $platforms_result->fetch_assoc()) {
                                    ...
                                    $platform_logo = htmlspecialchars($platform_row['logo'] ?? '');
                                    echo '<span class="custom-option" data-value="' . $platform_name . '" data-logo="' . $platform_logo . '">' . $platform_name . '</span>';
                                }
                            } else {
    ...
    if (mainCard && customSelect) {
        ...
        options.forEach(option => {
            option.addEventListener('click', function() {
                if (this.hasAttribute('data-value')) {
                    const logoFile = this.getAttribute('data-logo');
                    triggerSpan.textContent = this.textContent;
                    triggerSpan.style.color = 'var(--text-primary)';
                    hiddenInput.value = this.getAttribute('data-value');
                    if (logoFile) {
                        mainLogo.src = '<?= BASE_URL ?>images/platforms/' + logoFile;
                    } else {
                        mainLogo.src = defaultLogo;
                    }
        ...
        hiddenInput.addEventListener('input', () => {
            if (!hiddenInput.value) {
                triggerSpan.textContent = 'Selecciona una plataforma';
