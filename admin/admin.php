...
if ($server_count == 0) {
    ...
    $insert_stmt = $conn->prepare(
        "INSERT INTO email_servers (server_name, enabled, imap_server, imap_port, imap_user, imap_password) VALUES (?, ?, ?, ?, ?, ?)"
    );
...
// function escapeHtml(unsafe) { ... }
        ...
        <div class="tab-pane fade <?= $active_tab === 'platforms' ? 'show active' : '' ?>" id="platforms" role="tabpanel">
            <div class="admin-card">
                ...
                <?php
                $platforms_stmt = $conn->prepare("SELECT id, name, logo, created_at FROM platforms ORDER BY sort_order ASC");
                                    ...
                                    <tr data-id="<?= $platform['id'] ?>" class="sortable-item">
                                        <td>
                                            ...
                                            <?php if (!empty($platform['logo'])): ?>
                                                <img src="../images/platforms/<?= htmlspecialchars($platform['logo']) ?>" alt="Logo" class="platform-icon">
                                            <?php else: ?>
                                        ...
                                        <td>
                                            <div class="d-flex gap-sm">
                                                <button class="btn-admin btn-primary-admin btn-sm-admin" onclick="openEditPlatformModal(<?= $platform['id'] ?>, '<?= htmlspecialchars(addslashes($platform['name'])) ?>', '<?= htmlspecialchars(addslashes($platform['logo'] ?? '')) ?>')">
...
<div class="tab-pane fade <?= $active_tab === 'licencia' ? 'show active' : '' ?>" id="licencia" role="tabpanel">
    <div class="admin-card">
        ...
        <div class="p-3">
            <?php if (!empty($_SESSION['license_sync_message'])): ?>
                <div class="alert-admin alert-info-admin mb-3">
                    <i class="fas fa-info-circle"></i>
                    <span><?= htmlspecialchars($_SESSION['license_sync_message']) ?></span>
                </div>
                <?php unset($_SESSION['license_sync_message']); ?>
            <?php endif; ?>

            <?php if ($license_info): ?>
                <?php
                    $license_key_raw = $license_info['license_key'] ?? '';
                    $license_key_masked = $license_key_raw ? str_repeat('*', max(0, strlen($license_key_raw) - 4)) . substr($license_key_raw, -4) : 'N/A';
                ?>
                <div class="current-license-info" style="background: rgba(0,0,0,0.2); border-radius: 12px; padding: 1.5rem;">
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-6 text-md-end">
                            ...
                            <p class="mb-1"><strong class="text-secondary">Próxima Verificación:</strong><br><?= htmlspecialchars($license_info['last_check']) ?></p>
                            <p class="mb-1"><strong class="text-secondary">Tipo de Licencia:</strong><br><?= htmlspecialchars($license_info['license_type'] ?? 'N/A') ?></p>
                            <p class="mb-1"><strong class="text-secondary">Fecha de Vencimiento:</strong><br><?= htmlspecialchars($license_info['expires_at'] ?? 'N/A') ?></p>
                            <p class="mb-0"><strong class="text-secondary">Clave de Licencia:</strong><br><?= htmlspecialchars($license_key_masked) ?></p>
                        </div>
                    </div>
            ...
            <div class="text-center mt-4 pt-3 border-top border-secondary border-opacity-25">
                 <a href="sync_license.php" class="btn-admin btn-secondary-admin btn-lg-admin me-2">
...
<div class="modal fade modal-admin" id="addPlatformModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="addPlatformForm" method="POST" action="procesar_plataforma.php" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group-admin">
                        <label for="add_platform_logo" class="form-label-admin">
                            <i class="fas fa-image me-2"></i>
                            Logo de la Plataforma
                        </label>
                        <div class="d-flex align-items-center gap-2">
                            <img id="add_platform_logo_preview" class="platform-logo-preview" style="display:none;" alt="Vista previa"/>
                            <input type="file" class="form-control-admin" id="add_platform_logo" name="platform_logo" accept="image/png">
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
...
<div class="modal fade modal-admin" id="editPlatformModal" tabindex="-1">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <form id="editPlatformForm" method="POST" action="procesar_plataforma.php" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group-admin">
                        <label for="edit_platform_logo" class="form-label-admin">
                            <i class="fas fa-image me-2"></i>
                            Logo de la Plataforma
                        </label>
                        <div class="d-flex align-items-center gap-2">
                            <img id="edit_platform_logo_preview" class="platform-logo-preview" src="" alt="Logo actual" style="display:none;"/>
                            <div class="flex-grow-1">
                                <input type="file" class="form-control-admin" id="edit_platform_logo" name="platform_logo" accept="image/png">
                                <div class="form-check mt-2">
                                    <input class="form-check-input" type="checkbox" id="delete_platform_logo" name="delete_logo">
                                    <label class="form-check-label" for="delete_platform_logo">Eliminar logo</label>
                                </div>
                            </div>
                        </div>
                    </div>
...
// Vista previa del logo de plataforma
function previewPlatformLogo(input, previewElement) {
...
function openEditPlatformModal(platformId, platformName, platformLogo) {
    ...
    const logoPreview = document.getElementById('edit_platform_logo_preview');
    const deleteCheckbox = document.getElementById('delete_platform_logo');
    if (deleteCheckbox) deleteCheckbox.checked = false;
    if (platformLogo) {
        logoPreview.src = '<?= BASE_URL ?>images/platforms/' + platformLogo;
        logoPreview.style.display = 'block';
    } else {
        logoPreview.style.display = 'none';
    }
    const logoInput = document.getElementById('edit_platform_logo');
    if (logoInput) {
        logoInput.value = '';
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addInput && addPreview) {
        addInput.addEventListener('change', function() {
            previewPlatformLogo(this, addPreview);
        });
    }
    const editInput = document.getElementById('edit_platform_logo');
    const editPreview = document.getElementById('edit_platform_logo_preview');
    const deleteCheckbox = document.getElementById('delete_platform_logo');
    if (editInput && editPreview) {
        editInput.addEventListener('change', function() {
            previewPlatformLogo(this, editPreview);
            if (deleteCheckbox) deleteCheckbox.checked = false;
        });
    }
    if (deleteCheckbox && editPreview) {
        deleteCheckbox.addEventListener('change', function() {
            if (this.checked) {
                editPreview.style.display = 'none';
            } else if (editPreview.src) {
